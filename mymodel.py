...
# from msc.py
class MyMSC(nn.Module):
    """
    Multi-scale inputs
    """

    def __init__(self, base, scales=None):
        super(MyMSC, self).__init__()
        print("my msc")
        self.base = base
        if scales:
            self.scales = scales
...
class MyDeepLabV2_ASPP(nn.Sequential):
    """
    ...
    def __init__(self, n_classes, n_blocks, atrous_rates):
        super(MyDeepLabV2_ASPP, self).__init__()
        print("my deeplab v2 with aspp")
...
class MyDeepLabV2_NOASPP(nn.Sequential):
    """
    DeepLab v2: Dilated ResNet + ASPP
    Output stride is fixed at 8
    """
    ...
    def __init__(self, n_classes, n_blocks, atrous_rates):
        super(MyDeepLabV2_NOASPP, self).__init__()
        print("my deeplab v2 without aspp")
        ch = [64 * 2 ** p for p in range(6)]
        self.add_module("layer1", _Stem(ch[0]))
        self.add_module("layer2", _ResLayer(n_blocks[0], ch[0], ch[2], 1, 1))
        self.add_module("layer3", _ResLayer(n_blocks[1], ch[2], ch[3], 2, 1))
...
def MyDeepLabV2_NOASPP_ResNet101(n_classes):
    return MyMSC(
        base=MyDeepLabV2_NOASPP(
            n_classes=n_classes, n_blocks=[3, 4, 23, 3], atrous_rates=[6, 12, 18, 24]
        ),
        scales=[0.5, 0.75],
    )

def MyDeepLabV2_ASPP_ResNet101(n_classes):
    return MyMSC(
        base=MyDeepLabV2_ASPP(
            n_classes=n_classes, n_blocks=[3, 4, 23, 3], atrous_rates=[6, 12, 18, 24]
        ),
        scales=[0.5, 0.75],
    )